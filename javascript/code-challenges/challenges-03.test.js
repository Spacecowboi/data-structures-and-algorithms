...
const containsAnd = (arr) => {
  return arr.filter((ting) => ting.includes('and'));
};
...
const oddValues = (arr) => {
  return arr.filter((things) => things % 2 !== 0);
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter((forbiddenGems) => !forbiddenValues.includes(forbiddenGems));
};
